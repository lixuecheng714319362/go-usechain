...
const (
	...
	testInfo                 = "113b218c583e05d08130e04ddd9ff852095e3352ca79b0249385ce438035b3af"
)
...
func (s *PublicTxPoolAPI) Inspect() map[string]map[string]map[string]string {
	...
	var format = func(tx *types.Transaction) string {
		...
		return fmt.Sprintf("contract creation: %v hui + %v gas × %v hui", tx.Value(), tx.Gas(), tx.GasPrice())
	}
	// Flatten the pending transactions
	for account, txs := range pending {
...
func (s *PrivateAccountAPI) Verify(id string, photo string) (bool, error) {
	err := cacertreg.CAVerify(id, photo)
	if err != nil {
		return false, err
	}
	return true, nil
...
// GetBalance returns the amount of hui for the given address in the state of the
...
func (s *PrivateAccountAPI) NewABaccount(address common.Address, password string) (common.Address, error) {
	...
	acc, _, err := fetchKeystore(s.am).NewABaccount(account, password)
	if err == nil {
		return acc.Address, nil
	}
	return common.Address{}, err
