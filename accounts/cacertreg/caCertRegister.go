...
func geneCSR(idHex string) string {
	...
	savePEMKey(node.DefaultDataDir()+node.PathSeparator()+"userrsa.prv", keyBytes)
	savePublicPEMKey(node.DefaultDataDir()+node.PathSeparator()+"userrsa.pub", publicKey)
...
func queryID(CAserver string, idKey string) error {
	...
	userCert := node.DefaultDataDir() + node.PathSeparator() + "user.crt"
